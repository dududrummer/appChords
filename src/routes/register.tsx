      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
          {[1, 2].map((s) => (
            <div
              key={s}
              className={`h-2 rounded-full transition-all ${step === s ? "w-12 bg-neo-orange" : "w-6 bg-black/20"}`}
            />
          ))}
        </div>
        ...
        <NeubrutalistCard className="p-8">
          ...
          {success && (
            <div className="bg-green-50 border-2 border-green-600 px-4 py-2 mb-4 font-bold text-sm text-green-700">
              {success}
            </div>
          )}
          ...
          {step === 1 ? (
            <>
              ...
              >
                ...
                {authAction === "google" ? "Conectando..." : "Cadastrar com Google"}
              </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                  {avatarPreview ? (
                    <img
                      src={avatarPreview}
                      alt="Avatar"
                      className="w-24 h-24 rounded-full border-3 border-black object-cover"
                    />
                  ) : (
              ...
              <FormField
                ...
                onChange={(v) => updateForm("artisticName", v)}
                placeholder="Como quer ser chamado?"
                id="register-artistic-name"
              />
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField
                  ...
                  onChange={(v) => updateForm("phoneWhatsapp", v)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={form.age || ""}
                    onChange={(e) =>
                      updateForm("age", e.target.value ? parseInt(e.target.value) : undefined)
                    }
                    placeholder="Ex: 25"
                    className="w-full rounded-lg border bg-white px-4 py-3 font-medium text-sm placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-ring"
                    id="register-age"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border bg-white px-4 py-3 font-medium text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer"
                    id="register-gender"
                  >
                ...
                <div className="grid grid-cols-4 gap-2">
                  {INSTRUMENTS.map((inst) => (
                    <button
                      key={inst}
                      type="button"
                      onClick={() => updateForm("instrument", inst)}
                      className={`rounded-lg border px-2 py-2 text-xs font-semibold transition-all cursor-pointer ${form.instrument === inst ? "bg-primary text-primary-foreground shadow-sm" : "bg-white hover:bg-muted"}`}
                    >
                      {inst}
                    </button>
                  ))}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateForm("howDidYouFindUs", e.target.value)}
                  className="w-full rounded-lg border bg-white px-4 py-3 font-medium text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer"
                  id="register-source"
                >
          ...
          <p className="text-center mt-6 text-sm font-bold text-black/60">
            Já tem conta?{" "}
            <Link to="/login" className="text-neo-orange hover:underline font-black">
              Entrar
            </Link>
          </p>
        </NeubrutalistCard>
