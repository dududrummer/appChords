    ...
    <div className="min-h-screen bg-neo-bg flex flex-col items-center justify-center px-4 py-12 relative">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <NeubrutalistCard className="p-8">
          ...
          <p className="font-accent text-center text-black/50 mb-8">Acesse sua conta e continue estudando</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="font-heading text-xs tracking-wider uppercase text-black/60 block mb-1.5">Email</label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-black/30" />
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" className="w-full pl-10 pr-4 py-3 border-2 border-black bg-white font-medium text-sm placeholder:text-black/30 focus:outline-none focus:ring-2 focus:ring-neo-orange" id="login-email" />
              </div>
            </div>
            <div>
              <label className="font-heading text-xs tracking-wider uppercase text-black/60 block mb-1.5">Senha</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-black/30" />
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full pl-10 pr-12 py-3 border-2 border-black bg-white font-medium text-sm placeholder:text-black/30 focus:outline-none focus:ring-2 focus:ring-neo-orange" id="login-password" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-black/30 hover:text-black cursor-pointer">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 border-2 border-black accent-neo-orange" />
                <span className="text-xs font-bold text-black/60">Lembrar de mim</span>
              </label>
              <a href="#" className="text-xs font-bold text-neo-orange hover:underline">Esqueci a senha</a>
            </div>
            <NeubrutalistButton type="submit" size="lg" className="w-full" disabled={isLoading}>
              {authAction === "email" ? <><Loader2 className="h-5 w-5 animate-spin mr-2" />Entrando...</> : "Entrar"}
            </NeubrutalistButton>
          </form>
